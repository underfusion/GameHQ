...
<!-- CCGUI:TASK id=t22 status=needs_decision weight=1 progress=31 position=7 risk=high -->
- [ ] Phase 6 — HDR capture pipeline
  ...
  <!-- CCGUI:TASK id=t24 status=deferred weight=1 progress=85 parent_id=t22 position=1 risk=high -->
  - [ ] HDR-aware SDR capture with GPU tone mapping
    ...
    - Result: The experimental HDR capture path is implemented and tested, but not yet verified against real HDR hardware.
    - Note: Hidden behind internal.capture.experimental_hdr (default off). Deferred until someone tests it with Windows HDR active and a real HDR game per t22.
    - Agent notes: Implementation: FramePumpService::createSession now branches on hdrExperimentalEnabled (threaded from FramePumpService::startBuffer reading the config key) - when on AND HdrCapabilities::forWindow reports HDR-active AND GpuToneMapper::checkFp16Support(device) passes AND GpuToneMapper::init succeeds, the WGC frame pool is created DirectXPixelFormat_R16G16B16A16Float instead of B8G8R8A8; poll() then runs each frame through GpuToneMapper::apply() before handing the resulting BGRA8 texture to the unchanged SegmentRecorder::writeFrame. Any gate failing anywhere falls straight back to the original hardcoded BGRA8 pool creation - zero behavior change for flag-off/non-HDR users, confirmed by the existing non-HDR tests staying green. New files: src/capture/hdr/HdrToneMapMath.{h,cpp} (CPU oracle), GpuToneMapper.{h,cpp} (D3DCompile-based full-screen-triangle pixel shader, d3dcompiler linked). Curve: identity in [0,1] (SDR losslessly preserved), hard clip to white above 1.0, then sRGB OETF - plain Reinhard x/(1+x) was tried first and rejected: it maps SDR white 1.0 to 0.5 pre-encode (about 188/255), which would visibly dim all ordinary SDR content; a perceptual shoulder curve that also preserves SDR white needs real HDR content to tune, out of scope here. Two real bugs found and fixed via actual GPU testing on this dev machine (not just design review): (1) the Reinhard SDR-white regression above, caught by tst_hdrtonemap; (2) CreateInputLayout with a null/zero descriptor returns E_INVALIDARG on this driver despite being a documented pattern elsewhere - fixed by skipping input-layout creation entirely and binding IASetInputLayout(nullptr), caught by tst_hdrgputonemap actually running the shader pipeline. Tests: tst_hdrtonemap (10/10, pure CPU logic, always runs) plus tst_hdrgputonemap (3/3, opt-in - creates a real throwaway D3D11 device, uploads a synthetic FP16 texture, runs it through the real GpuToneMapper, reads back and compares to the CPU oracle within tolerance; QSKIPs cleanly with no D3D11 device or no FP16 support). Full suite 14/14 green. Committed 3d6d3f2, VERSION 0.6.15. Gap not yet built: t25's screenshot path needs its own one-shot WGC capture (ScreenshotService is GDI BitBlt, structurally cannot see HDR pixel data - confirmed by code research, not assumption). Next: get a human with Windows HDR plus a real HDR game to flip internal.capture.experimental_hdr on and confirm the SDR path is genuinely unaffected and the HDR path looks correct, then start t25 per t22's sequencing.
