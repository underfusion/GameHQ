...
- **Dialogs/popups**: `surface`, `radiusL`, dim scrim `bg0` @ 60 %; one primary action max. `AboutWhatsNewDialog` is a compact information surface, not a copy of Settings -> About: it owns release highlights and key actions while detailed update controls, storage mode and technical links remain in Settings.
