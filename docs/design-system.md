...
- **Dialogs/popups**: `surface`, `radiusL`, dim scrim `bg0` @ 60 %; one primary action max. `AboutWhatsNewDialog` is a compact information surface, not a copy of Settings -> About: it owns current release highlights and key actions while detailed update controls, storage mode and technical links remain in Settings. Its full release-notes view bundles a bounded recent history and lists newest-first version links across the top so skipped versions remain readable offline with mouse, keyboard, or controller.
