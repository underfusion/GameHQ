...
- `themes/Skin.qml` declares the skinnable color surface (Dark values as defaults); `DarkSkin`/`LightSkin`/`HighContrastSkin` override from it, so an omitted token falls back to Dark. `Theme.qml` binds every color token through the active skin and re-assigns `activeSkin` on `configChanged` (`theme.active_skin`), which repaints live. Sizes, fonts and durations are not skinnable. See docs/design-system.md §0.
